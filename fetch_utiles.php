...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['gradoID'])) {
    ...
    $sqlUtiles = "SELECT ID, Nombre, Descripcion, Cantidad FROM ListaDeUtiles WHERE GradoID = $gradoID";
    ...
    if ($resultUtiles->num_rows > 0) {
        ...
        while ($rowUtil = $resultUtiles->fetch_assoc()) {
            ...
            $utilesContainer .= '<input type="number" name="cantidad_' . $utilID . '" id="cantidad_' . $utilID . '" class="form-control" value="' . $cantidad . '" min="1">';
            $utilesContainer .= '</div>';
        }
